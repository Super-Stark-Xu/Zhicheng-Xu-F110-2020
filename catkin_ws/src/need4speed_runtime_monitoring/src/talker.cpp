...
public:
  ...
  void x_speed_callback(const vesc_msgs::VescStateStamped::ConstPtr& msg)
  { 
    double x = msg->state.speed;//get speed data
...
{
  ros::init(argc, argv, "need4speed_average_vesc");
	//the new node called need4speed_average_vesc
  ros::NodeHandle n;
  average_speed average;//class pointer
  ros::Subscriber sub = n.subscribe("sensors/core", 10, &average_speed::x_speed_callback, &average);
